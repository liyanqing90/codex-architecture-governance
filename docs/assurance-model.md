...
| A decision cites generic architectural fashion. | Schema 1.2 remediation and 1.3 Greenfield Decisions bind exact selected Markdown entry hashes and compare keep-current plus alternatives across quality scenarios, business/team/evolution fit, complexity, maturity, and lock-in. | Knowledge is guidance; local evidence and accountable judgment still decide. |
| A new-system decision fabricates findings to satisfy the remediation contract. | Greenfield decisions bind a validated Design Brief with quality scenarios, boundaries, critical flows, facts, assumptions, and unknowns; their Finding list must be empty. | The Design Brief can still contain mistaken owner assertions. |
