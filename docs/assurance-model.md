...
| A decision cites generic architectural fashion. | Schema 1.2 Decisions bind exact selected Markdown entry hashes and compare keep-current plus alternatives across quality scenarios, business/team/evolution fit, complexity, maturity, and lock-in. | Knowledge is guidance; local evidence and accountable judgment still decide. |
...
| Irrelevant knowledge overwhelms the task. | Selection binds repository facts, Profile, task, Skill, entry hashes, context budget, inclusion reasons, and exclusions. | Trigger quality and an explicit include can still select weak context. |
