...
or broad domain match is not a replacement test. An exact decision intent is
also an auditable exception. `--maintainer` is for curation/maintenance
...
authority, offline writes, synchronization, and conflict behavior. This keeps
the word `local-first` from crossing those two decision domains.
