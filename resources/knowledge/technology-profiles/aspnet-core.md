...
Use ASP.NET Core only for capabilities established by current official documentation and matched to a verified project requirement.
...
The project's language, runtime, deployment, and team constraints match ASP.NET Core's operating model.

## Avoid when

A simpler existing dependency meets the requirement or the team cannot own ASP.NET Core's lifecycle and failure modes.
