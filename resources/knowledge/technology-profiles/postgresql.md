---
id: technology.postgresql
kind: technology-profile
version: 1.0.0
status: active
domains:
- relational-database
triggers:
- postgresql
quality_attributes: []
related: []
legacy_ids:
- technology-profile:postgresql
last_reviewed: '2026-07-28'
review_after_days: 90
source_policy: official-docs-required
sources:
- title: PostgreSQL Documentation
  url: https://www.postgresql.org/docs/
  authority: official
dynamic_facts: true
version_range: Current supported stable releases; verify official documentation before a project
  decision.
---

# PostgreSQL

## Problem and intent

- Provide transactional relational storage
- constraints
- indexing
- SQL queries
- extensions
- and operational replication.

## Mechanism

- Provide transactional relational storage

## Fit when

- Relations
- transactions
- constraints
- flexible queries
- and mature tooling match the data model.

## Avoid when

- A specialized access model or scale cannot be met without harmful contortions.

## Required capabilities

- database-operations
- migration-discipline
- backup-restore

## Benefits

- Strong integrity
- rich SQL
- extensibility
- and broad operational support.

## Costs and liabilities

- Schema and query governance
- connection management
- vacuum
- replication
- and capacity ownership.

## Failure modes

- The mechanism is adopted by convention without a traced failure path.

## Alternatives

- mysql
- mongodb

## Migration and exit

- Introduce the mechanism behind a compatible boundary, verify it, then remove the old path.

## Evidence to inspect

- Trace the owning boundary, direct configuration or code, affected consumers, failure path, tests, and current operational evidence.
- For technology capabilities, confirm volatile behavior from the cited official source at decision time.

## Evidence that changes the recommendation

- A simpler option meeting the same measurable quality scenario should replace this recommendation.
- Missing ownership, compatibility, recovery, cost, or operational capability invalidates adoption until resolved.

## Quality trade-offs

- Balance business fit, reliability, maintainability, cost, and cognitive load.

## Volatile facts

- Product versions, support status, compatibility, security advisories, licensing, pricing, and service limits are time-sensitive and must be rechecked.
- Stable mechanism guidance remains separate from current vendor or release information.
