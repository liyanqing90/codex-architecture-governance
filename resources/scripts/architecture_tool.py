...
from select_knowledge import SelectionError, parse_kind_budget, select_knowledge
...
def validate_knowledge_selection_artifact(
    ...
    *,
    ...
    Schema 1.0/1.1 selections remain readable. New 1.2 selections bind their
    kind and maturity claims to the bundled Markdown source, and are
    deterministically replayed when their complete inputs are available.
    """
    ...
    for item in selection["selection"]:
        ...
        if entry is None:
            raise ArchitectureError(f"{path} selects unknown knowledge {item['id']}")
        expected = {
        ...
        if (
            selection["inputs"]["skill"] == "architecture-solution-advisor"
    ...
    replay_keys = {"maintainer_mode", "includes", "excludes"}
    if not replay_keys.issubset(inputs):
        return selection
    kind_budgets = {
