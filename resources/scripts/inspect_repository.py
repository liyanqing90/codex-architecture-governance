...
def _known_dependency_ids(values: list[str]) -> set[str]:
    text = "\n".join(values).lower()
    return {dependency for dependency in DEPENDENCY_FACTS if dependency in text}
...
def _setup_py_dependencies(text: str) -> set[str]:
    ...
    return _known_dependency_ids(dependencies)
...
def _manifest_dependencies(path: Path) -> set[str]:
    ...
    if name.startswith("requirements") and name.endswith(".txt"):
        return _known_dependency_ids(text.splitlines())
    if name == "pyproject.toml":
        ...
        if isinstance(project, dict):
            ...
            if isinstance(dependencies, list):
                result.update(
                    _known_dependency_ids([str(item) for item in dependencies])
                )
