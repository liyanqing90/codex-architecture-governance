...
def safe_extract(archive_path: Path, destination: Path) -> None:
    ...
    with zipfile.ZipFile(archive_path) as archive:
        ...
        for info in archive.infolist():
            if "\\" in info.filename:
                raise SmokeTestError(f"Archive entry uses a backslash: {info.filename}")
            relative = PurePosixPath(info.filename)
            if relative.is_absolute() or ".." in relative.parts:
                raise SmokeTestError(f"Unsafe archive entry: {info.filename}")
            mode = info.external_attr >> 16
            if stat.S_ISLNK(mode):
                raise SmokeTestError(f"Archive contains a symlink: {info.filename}")
            target = destination.joinpath(*relative.parts)
            try:
                target.resolve().relative_to(destination.resolve())
            except ValueError as exc:
                raise SmokeTestError(f"Unsafe archive entry: {info.filename}") from exc
            if info.is_dir():
...
def smoke_test(archive_path: Path) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="hengmu-package-smoke-") as temporary:
        ...
        try:
            manifest = json.loads(
                (unpacked / "plugin.json").read_text(encoding="utf-8")
            )
        except json.JSONDecodeError as exc:
            raise SmokeTestError(f"Portable manifest is invalid JSON: {exc}") from exc
        if not isinstance(manifest, dict):
            raise SmokeTestError("Portable manifest must be a JSON object")
        if manifest.get("$schema") != AGENT_PLUGINS_SCHEMA:
            raise SmokeTestError("Portable manifest has the wrong Agent Plugins schema")
