...
- `../../resources/schemas/knowledge-catalog.schema.json`;
...
- only the catalog files affected by the request.
...
Keep stable knowledge in catalogs. Record official-source URLs and a review
window for slow-changing knowledge. Verify fast-changing facts from current
official sources at use time; do not freeze them into a Skill.
...
3. Record:
   intent, fit conditions, avoid conditions, benefits, liabilities, required
   capabilities, warning signals, alternatives, operational requirements,
   migration paths, lock-in, sources, and freshness.
